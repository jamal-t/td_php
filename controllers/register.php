...
// Etape 4 > Vérifier que le login est unique (pas déjà dans la DB)
$resulats = findOneUserBy('email',$datas_form['email']);
...
if(count($messages) == 0 ) {
    //Exécuter une requêt SQL pour transférer les données saisies dans le form dans la base de données
...
// Etape 8 > Gestion des erreurs: Quand un/ou n problème(s) se déclenche(nt), il faut afficher tous les messages d'erreurs en même temps sur la page d'inscription.